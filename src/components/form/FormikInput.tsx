...
const FormikInput = ({ label, errorMessage, ...props }: { label?: string; errorMessage?: string } & FieldAttributes<any>) => {
  ...
  return (
    <div className="w-full">
      ...
      <Field
        {...props}
        className={`min-h-[44px] sm:min-h-[48px] w-full border rounded-lg px-3 sm:px-4 text-sm sm:text-base bg-white text-gray-800 placeholder-gray-400 outline-none transition-all duration-200 shadow-sm ${hasError
